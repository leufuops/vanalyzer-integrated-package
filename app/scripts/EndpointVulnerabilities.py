...
def parseEndpointVulnerabilities(apikey,urldashboard,jresponse): #endpointGroups):
    ...
    for i in jresponse['serverResponseObject']:
        ...
        vulid_raw = i['organizationEndpointVulnerabilitiesVulnerability']['vulnerabilityId']
        vulid = int(vulid_raw) if vulid_raw else 0
        link = f"https://www.vicarius.io/vsociety/vulnerabilities/{vulid}/{cve}"
...
# These functions use the aggregation API for more efficient vulnerability synchronization
...
def get_vulnerability_ids_by_endpoint(apikey, urldashboard, endpoint_hash, from_offset=0, size=100):
    ...
    try:
        print(f"[DEBUG] Fetching vulnerability IDs for endpoint_hash: {endpoint_hash}, offset: {from_offset}")
        ...
        print(f"[DEBUG] Response count: {server_response_count}, objects in page: {response_obj_count}")
...
def get_vulnerability_details_by_ids(apikey, urldashboard, vuln_ids, endpoint_hash, from_offset=0, size=100):
    ...
    try:
        print(f"[DEBUG] Fetching details for {len(vuln_ids)} vulnerability IDs")
        response = requests.get(f'{urldashboard}/vicarius-external-data-api/aggregation/searchGroup',
                              params=params, headers=headers)
        ...
        print(f"[DEBUG] Details response count: {server_response_count}, Objects: {server_response_objects}")
...
def parse_vulnerability_details(response):
    ...
    for vuln_obj in response['serverResponseObject']:
        ...
        # Extract data from aggregation API structure
        ...
        os_data = model_abs.get('organizationEndpointVulnerabilitiesOperatingSystem', {})
        ...
        # Extract vulnerability ID (keep as integer for DB compatibility)
        vulid_raw = vulnerability_data.get('vulnerabilityId')
        vulid = int(vulid_raw) if vulid_raw else 0
        link = f"https://www.vicarius.io/vsociety/vulnerabilities/{vulid}/{cve}" if vulid and cve else ""

        # Determine CVE type (App or OS)
        typecve = ""
        product_name = ""
        if product_data.get('productName'):
            product_name = product_data.get('productName', '')
            typecve = "App"
        elif os_data.get('operatingSystemName'):
            product_name = os_data.get('operatingSystemName', '')
            typecve = "SO"
        ...
        patch_name = patch_data.get('patchName', 'n\\a') if patch_id > 0 else 'n\\a'
        patch_release_date = patch_data.get('patchReleaseDate', 0)
        ...
        vulnerability_summary = vulnerability_summary.replace(",", "").replace(";", "").replace("'", "").replace('"', "'")

        # Create vulnerability dictionary (compatible with existing schema)
        vulnerability_dict = {
